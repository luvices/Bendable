...
/// Full-frame imagery generated by the shared Metal pass instead of sampled from a
/// captured desktop. Parameters still live in `FrameDescription`, so previews,
/// render sheets and the live hinge all evaluate the exact same frame.
enum ProceduralEffect: UInt32, Sendable, Equatable {
    case none = 0
    case sunsetHDR = 1
}
...
struct FrameDescription: Sendable, Equatable {
    ...
    var proceduralEffect: ProceduralEffect = .none
    /// Normalized close amount supplied directly to a procedural effect.
    var effectProgress: Double = 0
